...
export default class Equipments extends Service {
  ...
  public async uploadPic(files: any, id: string, isCover: string) {
    const basePath = './picsData/';
    for (const file of files) {
      const { filename, filepath } = file;
      const target = basePath + filename;
      this.app.logger.info(`正在上传文件 '${filename}' 到 '${target}' `);
      const useTarget = `http://10.10.203.60:7002/${filename}`
      copyFile(filepath, target, true);
      if (isCover === 'true') {
        await this.app.mysql.query(`update equipment set avatar_path = '${useTarget}' where id = '${id}'`);
      } else {
   ...
   */
 public async getPicsById(id:string){
  let result: EggMySQLSelectResult;
    if (id) {
      this.app.logger.info(`正在获取设备id为${id}的设备图片`);
      result = await this.app.mysql.select('equipment_pic', {
        where: {
          id,
        },
      });
